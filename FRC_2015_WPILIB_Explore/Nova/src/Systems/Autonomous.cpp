...
void AutonomousSystem::Run3ToteAuto(MecanumDrive *drive, LiftSystem *lifter){
	...
	if(autoMode == Reset){
		if(lifter->longBottomLiftSystem->Get() == true){
		...
		else if(lifter->longBottomLiftSystem->Get() == false){
			LifterSetpoint = -1;
		}
	}
	...
	if(autoMode == Stack1Tote){
		...
		if(((LifterDistance >= 0 && LifterDistance <= 95) && WheelEncoder >= -20)){//drive back to wheel <= -20 lift <= 0. set mode to step3
			...
			if(lifter->longBottomLiftSystem->Get() == false){
				LifterSetpoint = -1;
			}
		}
	...
	if(autoMode == Lift2Totes){
		...
		if(lifter->shortToteTouchSensor->Get() == false){//drive forward to 2nd tote.
			drive->AutonDriveStraight(true, 0.5);
			drive->FLWheel->Reset();
		}
	...
	if(autoMode == Lift3Totes){
		...
		if(lifter->shortToteTouchSensor->Get() == false){//drive forward to tote stack.
			drive->AutonDriveStraight(true, 0.5);
			drive->FLWheel->Reset();
		}
		...
		if(WheelEncoder <= -200 && (LifterDistance >= 0 && lifter->longBottomLiftSystem->Get() == false)){
			LifterSetpoint = -0.5;
		}

		if(WheelEncoder <= -200 && lifter->longBottomLiftSystem->Get()){
			autoMode = DriveToAutoZone;
		}
	}

	if(autoMode == DriveToAutoZone && lifter->longBottomLiftSystem->Get() && WheelEncoder >= -300){
