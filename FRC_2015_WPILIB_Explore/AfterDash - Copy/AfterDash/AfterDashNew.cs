    ...
    public partial class AfterDash : Form
    {
        ...
        public void DisplayData(string stringData)
        {
            ...
            for (int i = 0; i < specific_packets.Length; i++)
            {
                ...
                #region Sent Data
                if(packet[0].Equals("SN"))
                {
                    SonicFeet.Invoke(new MethodInvoker(delegate { SonicFeet.Text = (Convert.ToDouble(packet[1].ToString())).ToString(); }));
                }
                if (packet[0].Equals("POTS"))
                {
                    POTSetpoint.Invoke(new MethodInvoker(delegate { POTSetpoint.Text = (Convert.ToDouble(packet[1].ToString())).ToString(); }));
                }
                /*if (packet[0].Equals("TRLS"))
                {
                    TopSwitch.Invoke(new MethodInvoker(delegate { TopSwitch.Text = (Convert.ToDouble(packet[1].ToString())).ToString(); }));
                }
                if (packet[0].Equals("BRLS"))
                {
                    BotSwitch.Invoke(new MethodInvoker(delegate { BotSwitch.Text = (Convert.ToDouble(packet[1].ToString())).ToString(); }));
                }
                if (packet[0].Equals("SL"))
                {
                    slideTouch.Invoke(new MethodInvoker(delegate { slideTouch.Text = (Convert.ToDouble(packet[1].ToString())).ToString(); }));
                }
                /*if (packet[0].Equals("PV"))
                {
                    potVal.Invoke(new MethodInvoker(delegate { potVal.Text = (Convert.ToDouble(packet[1].ToString())).ToString(); }));
                }
                if (packet[0].Equals("ED"))
                {
                    encDis.Invoke(new MethodInvoker(delegate { encDis.Text = (Convert.ToDouble(packet[1].ToString())).ToString(); }));
                }*/
